...
public class UserController {
	...
	@ResponseBody
	public String login(@Param("phone") String phone, @Param("password") String password, HttpServletRequest request) {
		User user = new User();
		user.setPhone(phone);
	...
	@ResponseBody
	public String deteteByIds(String[] ids, HttpServletRequest request) {
		HttpSession session = request.getSession();
		String roleid = (String) session.getAttribute(Constant.SESSION_USER_ROLE);
		if (roleid.equals("1")) {
			if (ids.length > 0) {
				User user = null;
				for (String id : ids) {
